...
public class CashierController {
    ...
    public void initialize() {
        ...
        colQty.setOnEditCommit(event -> {
            ...
            if (newQty != null && newQty > 0) {
                ...
                updateUI(); // Recalculate totals immediately
            } else {
                // Invalid (negative/zero), revert changes
                cartTable.refresh();
            }
        });
    ...
    private void handleAddItem() {
        try {
            saleManager.addItemToSale(barcodeField.getText().trim(), 1);
            barcodeField.clear();
            updateUI();
        } catch (Exception e) {
